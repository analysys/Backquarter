...
import com.yiguan.kafka.util.DistinctAST;
 ...
 * @author Europe
 */
public class App202Consumer implements Runnable {
	...
	public App202Consumer(String topic, String group, Logger logger) throws Exception {
	...
	public void readKafak() {
		...
		while (true) {
			...
			while (it.hasNext()) {
				...
				try {
					String distinctJSON = DistinctAST.distinctAll(new String(dataByte));
					logger.info(OZBase64.encode(distinctJSON.getBytes()));
					count++;
					if (count % 50000 == 0) {
